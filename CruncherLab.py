...
def main(argv):
    ...
    groupby_max = True
    groupby_min = True
    data_partials = [1.0, 0.6, 0.15]
    test_data_partials = [1.0, 0.6, 0.15]
    ...
    for subset_locations in subset_locations_values:
        prepare_full_data_scenarios(subset_wifi_samples_locations(wifi_samples, 0.5), data_scenarios,
                                    raw=raw,
                                    groupby_mean=groupby_mean,
                                    groupby_max=groupby_max,
                                    groupby_min=groupby_min,
    ...
    save_scenarios(data_scenarios, prefix="train_")
    print("# Data Scenarios: " + str(len(data_scenarios)))
    save_scenarios(test_data_scenarios, prefix="test_")
    ...
    metrics.to_csv(output_data_directory + "/metrics-k_neighbors=" + k_neighbors + ".csv")
